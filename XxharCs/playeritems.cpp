...
float PlayerItems::CurWallPierce(void)
{
	...
	{
		if (g_local.spread.WeaponState & M4A1_SILENCER) // note me should not be in here but as long as g_local.spread.WeaponState has the data for the current weapon it is ok
...
float PlayerItems::CurWallPierce(void)
{
	...
	if (weaponList[index].iId == WEAPONLIST_M4A1)
	{
		if (IsCurWeaponSilenced()) // note me should not be in here but as long as g_local.spread.WeaponState has the data for the current weapon it is ok
			return weaponList[index].wallpierce2;
		else
			return weaponList[index].wallpierce1;
	}
