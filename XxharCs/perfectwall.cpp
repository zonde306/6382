...
int CanPenetrate(const float *start, const float *end, float flDistance, int penetration, int bullettype, int iDamage, float wallpierce)
{
	...
	//gEngfuncs.pfnAngleVectors(g_local.ViewAngles, forward, right, up);
	...
	while (tmppen)
	{
		...
		if (tr.fraction != 1.0)
		{
			...
			if (/*pent && pent->solid == SOLID_BSP && */tmppen)
			{
				...
				//if (damage > 0 && pent->info == g_playerList[index].index)
				// return damage;
			...
			else
			{
				...
				//if (damage > 0 && pent && pent->info == g_playerList[index].index)
				// return damage;
